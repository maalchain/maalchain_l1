...
func PrepareEthTx(
	...
	signer := ethtypes.LatestSignerForChainID(appEthermint.EvmKeeper.ChainID())
	txFee := sdk.Coins{}
	txGasLimit := uint64(0)
	...
	for _, m := range msgs {
		...
		txFee = txFee.Add(sdk.Coin{Denom: evmtypes.DefaultEVMDenom, Amount: sdkmath.NewIntFromBigInt(msg.GetFee())})
	}
...
func CreateEthTx(
	...
	chainID := appEthermint.EvmKeeper.ChainID()

	// When we send multiple Ethereum Tx's in one Cosmos Tx, we need to increment the nonce for each one.
	nonce := appEthermint.EvmKeeper.GetNonce(ctx, fromAddr) + uint64(nonceIncrement)
	evmTxParams := &evmtypes.EvmTxArgs{
		...
		Amount:    amount,
		GasLimit:  100000,
		GasFeeCap: appEthermint.FeeMarketKeeper.GetBaseFee(ctx),
